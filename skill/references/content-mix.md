...
Each row records the four picks, metadata, and review status:
...
- date: 2026-04-24
  ...
  wordcount: 180
  status: accepted               # accepted | pending-review | rejected | regenerating
```
...
`status` is required for posts generated by v1.3.0+:
- **accepted** — user typed `/thoth` interactively OR ran `/thoth inbox accept`. The post is real and counts toward everything.
- **pending-review** — scheduled run wrote the draft to `inbox/`; user hasn't decided yet. **Excluded** from `/thoth calendar` ratio math and from the rotation windows (framework / hook last-N lists). Treated as if it didn't happen for accounting purposes.
- **rejected** — user explicitly rejected via `/thoth inbox reject`. Excluded from ratio math but kept in history for the record.
- **regenerating** — transient state while a regenerate runs against an inbox draft.

For older rows without `status`, treat as `accepted` (legacy interactive posts predate the inbox flow).

### Step 1 — Pick the type

Compute the percentages across the **last 20 posts with `status: accepted`** (or all such posts, if fewer than 20). Rows with `status: pending-review` or `rejected` are skipped — only posts the user has decided to ship count toward the ratio.
...
1. From the persona's `history.yaml`, list frameworks used in the last 4 posts with `status: accepted` (configurable — call this the *recent-frameworks window*). `pending-review` and `rejected` rows are skipped so the rotation doesn't lock onto a framework the user never shipped.
...
1. From the persona's `history.yaml`, list hook patterns used in the last 3 posts with `status: accepted` (configurable). Skip `pending-review` and `rejected` rows.
